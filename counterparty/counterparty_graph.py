...
class CounterpartyGraph:
    ...
    def __init__(
        ...
        for col_name in ["source", "target"]:
            cifs = (
                self.edges.filter(F.col("target_is_internal") | (F.col(col_name) == F.col("source")))
                .select(col_name)
                .distinct()
                .collect()
            )
            edge_cifs.update(r[0] for r in cifs)
        # Simpler: collect all source + internal targets
        ...
        logger.info(
            f"[CounterpartyGraph] Built graphs for {len(self._results)} customers "
            f"in {self._metrics.compute_time_seconds:.1f}s"
        )
    ...
    def save(self, path: str) -> None:
        ...
        if self.edges is not None:
            self.edges.write.mode("overwrite").parquet(
                os.path.join(path, "edges.parquet")
            )

        # Node attributes
        if self.nodes is not None:
            self.nodes.write.mode("overwrite").parquet(
    ...
    def visualize(
        ...
        from .graph_viz import visualize as _viz
...
def _build_edge_table(
    ...
    edges = (
        ...
        .filter(F.col("source") != F.col("target"))  # Remove self-loops
    )
